...
if hex(getnode()) in ["0xdc7b23bb434e"]: #windows masaüstü pc ise veya laptop
    #kaç yaşındasın bot
    print("kyb")
    bot_adi = "@Gaye_KelimeBot"
    bot_token = "7557773843:AAHc0Sc3SBl-kTHn_a_obA7iTjhLEWtIoCw"
...
kurucu_id = 7092148989

admins = [kurucu_id, 7187375734]
...
async def log_gonder(**kwargs):
    ...
    try:
        await bot.send_message(-1002243530183, f"""
<b> ~~ 📢 Log Kaydı ~~</b>
...
Eylem: <code>{kwargs.get('eylem','')}</code>
    ...
    """, disable_web_page_preview=True)
        ...
        #    bot.send_message(kurucu_id, str(e))
    try:
        await bot.set_chat_title(--1002243530183, f"Bot Log - {len(oyunlar)}")
    except Exception as e:
        if "chat not found" in str(e):
            pass
...
async def start_private(message): #, **kwargs
    ...
    if chat_tipi == "private":
        ...
        if len(ayrik) == 2:
            ...
            callback_button = types.InlineKeyboardButton(text="🇹🇷 ʙᴇɴɪ ɢʀᴜʙᴀ ᴇᴋʟᴇ 🇹🇷", url="https://t.me/Gaye_KelimeBot?startgroup=a")
...
async def callback_inline(cagri): #çağrıcı cagrici
    ...
    if "group" in msg_type:
        ...
        if f(f"groups.{chat_id}.username") == "":
            await bot.send_message(-1002243530183, f"📜 {grup_username} ⟶ {len(f('groups')) + 1}")
...
🙏 Yardım ve sorularınız için: @Merdobey
""")
